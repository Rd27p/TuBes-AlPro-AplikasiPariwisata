...
func cariBerdasarkanJarak(A arrWisata, n int) { // pass by value procedure
	...
	for i = 0; i < n-1; i++ {
		Idx = i
		for j = i + 1; j < n; j++ {
			if (descending && A[j].jarak > A[Idx].jarak) || (!descending && A[j].jarak < A[Idx].jarak) {
				Idx = j
			}
		}
		A[i], A[Idx] = A[Idx], A[i]
